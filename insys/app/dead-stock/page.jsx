...
import { Package, AlertTriangle, TrendingDown, Calendar, IndianRupee } from "lucide-react";

export default function DeadStockPage() {
  const [daysFilter, setDaysFilter] = useState(30);
  const allProducts = useQuery(api.offStore.getProductsForDeadStock) || [];
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 pt-12 lg:pt-0">
            <div>
              ...
              <p className="text-gray-500 text-sm mt-1">Products with no sales</p>
            </div>
            <select
              value={daysFilter}
          ...
          {deadStock.length === 0 ? (
            <div className="bg-white rounded-xl border p-12 text-center">
              ...
              <p className="text-sm text-gray-400 mt-1">All products have sales or are newer than {daysFilter} days</p>
            </div>
          ) : (
                ...
                <tbody className="divide-y">
                  {deadStock.map((product) => (
                    <tr key={product._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm text-right font-medium">{product.currentStock}</td>
