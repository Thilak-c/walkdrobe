...
import { Upload, Download, CheckCircle, XCircle, Globe, Store } from "lucide-react";

export default function OfflineImportPage() {
  const [activeTab, setActiveTab] = useState("file"); // "file" or "website"
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 p-4 lg:p-8">
        <div className="max-w-5xl mx-auto pt-12 lg:pt-0">
          ...
          <p className="text-gray-600 mb-6">Import products from file or website store</p>
          ...
          {activeTab === "file" ? <FileImportTab /> : <WebsiteImportTab />}
        </div>
      </main>
    </div>
  );
...
function FileImportTab() {
  ...
  const downloadTemplate = (type) => {
    if (type === "json") {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = "offline_import_template.csv";
      a.click();
    }
  };
...
function WebsiteImportTab() {
  ...
  const selectAll = () => {
    if (selected.length === websiteProducts.length) {
      setSelected([]);
    } else {
      setSelected(websiteProducts.map(p => p._id));
    }
  };
  ...
  const handleImport = async () => {
    ...
    const productsToImport = websiteProducts
    ...
    try {
      const res = await fetch("/api/import-products", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ products: productsToImport, store: "offline" }),
      });
      const result = await res.json();
      setResults(result);
      setSelected([]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-gray-600">Select products from your website store to import to offline inventory</p>
        <div className="flex gap-2">
          <button onClick={selectAll} className="px-3 py-1.5 text-sm border rounded hover:bg-gray-50">
            {selected.length === websiteProducts.length ? "Deselect All" : "Select All"}
          </button>
          <button
            onClick={handleImport}
      ...
      <div className="grid gap-3">
        ...
        {websiteProducts.length === 0 && (
          <div className="text-center py-12 text-gray-500">
            ...
            <p>No products in website store</p>
          </div>
        )}
      </div>
    </div>
