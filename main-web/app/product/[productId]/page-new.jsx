...
export default function ProductPage() {
  ...
  const allImages = product ? [product.mainImage, ...(product.otherImages || [])] : [];
    ...
    <div className="min-h-screen bg-white">
          ...
          <div className="space-y-6">
            ...
            {reviewStats && reviewStats.totalReviews > 0 && (
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-1">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} className={`w-4 h-4 ${i < Math.round(reviewStats.averageRating) ? "text-yellow-400 fill-current" : "text-gray-200"}`} />
                  ))}
                </div>
            ...
            <div className="flex gap-3 pt-4">
              <button
                ...
                className="flex-1 h-14 bg-white border-2 border-black text-black rounded-full font-semibold hover:bg-gray-50 transition-colors flex items-center justify-center gap-2"
              >
                <ShoppingCart className="w-5 h-5" />
                Add to Cart
              </button>
              <button
                ...
                className="flex-1 h-14 bg-black text-white rounded-full font-semibold hover:bg-gray-800 transition-colors"
              >
                Buy Now
              </button>
            </div>
          ...
          <section className="mt-16 pt-12 border-t border-gray-100">
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <p className="text-gray-500 text-sm mt-1">{reviewStats?.totalReviews || 0} reviews</p>
              </div>
              {reviewStats && (
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} className={`w-5 h-5 ${i < Math.round(reviewStats.averageRating) ? "text-yellow-400 fill-current" : "text-gray-200"}`} />
                    ))}
                  </div>
                </div>
              )}
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-gray-200 rounded-full flex items-center justify-center">
                        <span className="text-sm font-medium text-gray-600">{review.userName?.charAt(0)?.toUpperCase()}</span>
                      </div>
                      <div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4 z-40">
        <div className="flex gap-3">
          <button
            onClick={handleAddToCart}
            className="flex-1 h-12 bg-white border-2 border-black text-black rounded-full font-semibold text-sm"
          >
            Add to Cart
          </button>
          <button
            onClick={handleBuyNow}
            className="flex-1 h-12 bg-black text-white rounded-full font-semibold text-sm"
          >
            Buy Now
          </button>
        </div>
